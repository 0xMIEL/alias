...
export class UserController {
  ...
  async register(req: Request, res: Response, next: NextFunction) {
    ...
    const newUser = await this.userService.createUser({...req.body, password:hashedPassword});
  ...
  async login(req: Request, res: Response, next: NextFunction) {
    ...
    res.status(HTTP_STATUS_CODES.UNAUTHORIZED_401).json({ status: 'fail', message: 'Invalid credentials' });
  }}
}
