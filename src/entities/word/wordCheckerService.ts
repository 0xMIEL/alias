...
import { getRandomElement, calculateLevenshteinSimilarity, levenshtein } from './helpers/wordCheckerHelpers';
...
export class WordCheckerService {
  ...
  async checkSimilarity(inputWord: string, targetWord: string): Promise<number> {
    if (!inputWord || !targetWord) {
      throw new AppError('Both input and target words must be provided.');
    }
    const similarity = await calculateLevenshteinSimilarity(inputWord, targetWord);
  ...
  async isSimilarEnough(inputWord: string, targetWord: string, threshold: number): Promise<boolean> {
